...
def get_model(shape):
    model = Sequential()
    model.add(Convolution2D(36, 5, strides=(2, 2), padding="same", activation='relu',input_shape=shape))
    model.add(BatchNormalization())
    model.add(Convolution2D(48, 5, strides=(2, 2), activation='relu',padding="same"))
    model.add(Activation('relu'))
    model.add(BatchNormalization())
    model.add(Convolution2D(64, 5, strides=(2, 2), activation='relu',padding="same"))
    model.add(Activation('relu'))
    model.add(BatchNormalization())
    model.add(Convolution2D(96, 3, activation='relu',padding="same"))
    model.add(Activation('relu'))
    model.add(BatchNormalization())
    model.add(Convolution2D(96, 3, activation='relu',padding="same"))
    model.add(Activation('elu'))
...
def two_input_model(cam_shape, speed_shape):
    b = Input(shape=cam_shape)
    c = Input(shape=speed_shape)
